 ...
 *
 * <p>X509KeyManager which allows selection of a specific keypair and certificate chain (identified
 * by their keystore alias name) to be used by the server to authenticate itself to SSL clients.
 *
 * <p>This class is only used on Java 1.4 systems, on Java 1.5 and newer the @see {@link
 * ExtendedAliasKeyManager} is used instead
 *
 * <p>Based of org.apache.tomcat.util.net.jsse.JSSEKeyManager.
 *
...
public final class AliasKeyManager implements X509KeyManager {
   ...
   * @param issuers The list of acceptable CA issuer subject names, or null if it does not matter
   *     which issuers are used (ignored)
   * @param socket The socket to be used for this connection. This parameter can be null, in which
   *     case this method will return the most generic alias to use (ignored)
