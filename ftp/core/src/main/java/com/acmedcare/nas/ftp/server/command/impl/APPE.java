 ...
 * APPE &lt;SP&gt; &lt;pathname&gt; &lt;CRLF&gt;</code><br>
 *
 * <p>This command causes the server-DTP to accept the data transferred via the data connection and
 * to store the data in a file at the server site. If the file specified in the pathname exists at
 * the server site, then the data shall be appended to that file; otherwise the file specified in
    ...
    try {
      ...
      session.write(
          LocalizedFtpReply.translate(
              session, request, context, FtpReply.REPLY_150_FILE_STATUS_OKAY, "APPE", fileName));
