...
public class ServerBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
  ...
  protected void doParse(
    ...
    for (Element childElm : childs) {
      ...
      if ("listeners".equals(childName)) {
        ...
        Object userManager =
            parserContext.getDelegate().parseCustomElement(childElm, builder.getBeanDefinition());
        ...
        factoryBuilder.addPropertyValue(
            "userManager", SpringUtil.parseSpringChildElement(childElm, parserContext, builder));
        ...
        factoryBuilder.addPropertyValue(
            "fileSystem", SpringUtil.parseSpringChildElement(childElm, parserContext, builder));
    ...
    if (StringUtils.hasText(element.getAttribute("anon-enabled"))) {
      connectionConfig.setAnonymousLoginEnabled(
          SpringUtil.parseBoolean(element, "anon-enabled", true));
    }
  ...
  private MessageResource parseMessageResource(
  ...
  private Map<?, ?> parseFtplets(
    ...
    if (childs.size() > 0 && childs.get(0).getLocalName().equals("map")) {
      ...
      return parserContext
          .getDelegate()
          .parseMapElement(childs.get(0), builder.getBeanDefinition());
    } else {
      ManagedMap ftplets = new ManagedMap();
      for (Element ftpletElm : childs) {
        ftplets.put(
            ftpletElm.getAttribute("name"),
  ...
  private Map<?, ?> parseListeners(
      final Element listenersElm,
      final ParserContext parserContext,
      final BeanDefinitionBuilder builder) {
    ...
    for (Element listenerElm : childs) {
      ...
      if ("nio-listener".equals(ln)) {
        listener =
            parserContext
                .getDelegate()
                .parseCustomElement(listenerElm, builder.getBeanDefinition());
