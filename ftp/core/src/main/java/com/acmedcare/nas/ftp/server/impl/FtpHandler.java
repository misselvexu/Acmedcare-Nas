...
public interface FtpHandler {
   ...
   * #sessionCreated(FtpIoSession)}. The biggest difference from {@link
   * #sessionCreated(FtpIoSession)} is that it's invoked from other thread than an I/O processor
   * thread once thread modesl is configured properly.
   */
  ...
  /**
   * Invoked with the related {@link IdleStatus} when a connection becomes idle. This method is not
   * invoked if the transport type is UDP; it's a known bug, and will be fixed in 2.0.
   */
