...
/**
 * Defines an client session with the FTP server. The session is born when the client connects and
 * dies when the client disconnects. Ftplet methods will always get the same session for one user
 * and one connection. So the attributes set by <code>setAttribute()</code> will be always available
 * later unless that attribute is removed or the client disconnects.
 *
...
public interface FtpSession {
  ...
  /**
   * Returns maximum idle time. This time equals to {@link
   * ConnectionManagerImpl#getDefaultIdleSec()} until user login, and {@link User#getMaxIdleTime()}
   * after user login.
   *
  ...
  /**
   * Set maximum idle time in seconds. This time equals to {@link
   * ConnectionManagerImpl#getDefaultIdleSec()} until user login, and {@link User#getMaxIdleTime()}
   * after user login.
   *
   * @param maxIdleTimeSec The number of seconds the client is allowed to be idle before
   *                       disconnected.
   */
  ...
  /**
   * Stores an attribute in this request. It will be available until it was removed or when the
   * connection ends.
   *
   * @param name  The attribute name
  ...
  /**
   * Get the unique ID for this session. This ID will be maintained for the entire session and is
   * also available to MDC logging using the "session" identifier.
   *
