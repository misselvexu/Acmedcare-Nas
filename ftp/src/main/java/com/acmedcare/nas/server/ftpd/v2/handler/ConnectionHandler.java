...
package com.acmedcare.nas.server.ftpd.v2.handler;

import com.acmedcare.nas.server.ftpd.v2.FTPConnection;
import com.acmedcare.nas.server.ftpd.v2.FTPServer;
import com.acmedcare.nas.server.ftpd.v2.Utils;
import com.acmedcare.nas.server.ftpd.v2.api.UserAuthenticator;
import com.acmedcare.nas.server.ftpd.v2.api.UserAuthenticator.AuthException;
...
public class ConnectionHandler {
  ...
  public void onConnected() throws IOException {
    UserAuthenticator auth = con.getServer().getAuthenticator();
  ...
  public void registerCommands() {
    ...
    con.registerCommand(
        "AUTH", "AUTH <mechanism>", this::auth, false); // Security Mechanism (RFC 2228)
    con.registerCommand(
        "PBSZ", "PBSZ <size>", this::pbsz, false); // Protection Buffer Size (RFC 2228)
    con.registerCommand(
        "PROT", "PROT <level>", this::prot, false); // Data Channel Protection Level (RFC 2228)

    con.registerCommand("LPSV", "LPSV", this::lpsv); // Long Passive Mode (RFC 1639) (Obsolete)
    con.registerCommand(
        "LPRT", "LPRT <address>", this::lprt); // Long Active Mode (RFC 1639) (Obsolete)
    ...
    con.registerCommand(
        "HOST", "HOST <address>", this::host, false); // Custom Virtual Hosts (RFC 7151)
  ...
  private void acct(String info) {
    ...
    // The authenticator can also handle special formatted usernames, if really needed (for
    // instance: "username|account")
  ...
  private void pasv() throws IOException {
    ...
    passiveServer =
        Utils.createServer(0, 5, server.getAddress(), server.getSSLContext(), secureData);
    passive = true;
  ...
  private void auth(String mechanism) throws IOException {
    ...
    if (mechanism.equals("TLS")
        || mechanism.equals("TLS-C")
        || mechanism.equals("SSL")
        || mechanism.equals("TLS-P")) {
  ...
  private boolean authenticate(UserAuthenticator auth, String password) {
    try {
