...
public class Utils {
  ...
  // Time
  private static final SimpleDateFormat mdtmFormat =
  ...
  public static <F> String format(FileSystem<F> fs, F file) {
    // Intended Format
  ...
  public static <F> String getPermission(FileSystem<F> fs, F file) {
    // Intended Format
    // -rw-rw-rw-
    // -rwxrwxrwx
    // drwxrwxrwx
  ...
  public static <F> String getFacts(FileSystem<F> fs, F file, String[] options) {
    // Intended Format
  ...
  public static void write(OutputStream out, byte[] bytes, int len, boolean ascii)
      throws IOException {
    if (ascii) {
      // ASCII - Add \r before \n when necessary
      byte lastByte = 0;
  ...
  public static <F> InputStream readFileSystem(FileSystem<F> fs, F file, long start, boolean ascii)
      throws IOException {
    if (ascii && start > 0) {
  ...
  public static ServerSocket createServer(
      int port, int backlog, InetAddress address, SSLContext context, boolean ssl)
      throws IOException {
    if (ssl) {
