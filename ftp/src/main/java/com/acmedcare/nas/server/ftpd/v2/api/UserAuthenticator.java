...
package com.acmedcare.nas.server.ftpd.v2.api;
...
/**
 * Represents an user authenticator. You can implement your existing user database
 *
...
public interface UserAuthenticator {
   ...
   *
   * <p>The client will not send the host address if {@link #needsUsername(FTPConnection)} returns
   * {@code false} or the client does not support custom hosts.
   *
   ...
   *
   * <p>Only affects when {@link #needsUsername(FTPConnection)} is also {@code true}
   *
   * @param con      The FTP connection
   ...
   *
   * <p>You can use a custom file system depending on the user.
   *
   * <p>If the {@code host} is {@code null}, you can use a "default host" or a union of all hosts
   * combined as some clients might not support custom hosts.
   *
   * @param con      The FTP connection
   * @param host     The host address or {@code null} when the client didn't specified the hostname
   * @param username The username or {@code null} when {@link #needsUsername(FTPConnection)} returns
   *                 false
   * @param password The password or {@code null} when {@link #needsPassword(FTPConnection, String,
   *                 InetAddress)} returns false
   * @return A file system if the authentication succeeded
   * @throws AuthException When the authentication failed
   */
  FileSystem authenticate(FTPConnection con, InetAddress host, String username, String password)
