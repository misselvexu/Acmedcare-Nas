...
import com.acmedcare.nas.server.ftpd.v2.api.ResponseException;
...
public class FileHandler {
  ...
  public void registerCommands() {
    ...
    con.registerCommand(
        "XCWD", "XCWD <file>", this::cwd); // Change Working Directory (RFC 775) (Obsolete)
    con.registerCommand(
        "XCUP", "XCUP", this::cdup); // Change to Parent Directory (RFC 775) (Obsolete)
    con.registerCommand(
        "XPWD", "XPWD", this::pwd); // Retrieve Working Directory (RFC 775) (Obsolete)
    ...
    con.registerCommand(
        "MFMT",
        "MFMT <time> <file>",
        this::mfmt); // Change Modified Time (draft-somers-ftp-mfxx-04)

    con.registerCommand(
        "MD5", "MD5 <file>", this::md5); // MD5 Digest (draft-twine-ftpmd5-00) (Obsolete)
    con.registerCommand(
        "MMD5",
        "MMD5 <file1, file2, ...>",
        this::mmd5); // MD5 Digest (draft-twine-ftpmd5-00) (Obsolete)

    con.registerCommand(
        "HASH", "HASH <file>", this::hash); // Hash Digest (draft-bryan-ftpext-hash-02)
  ...
  private void retr(String path) throws IOException {
    ...
    con.sendResponse(
        150, "Sending the file stream for " + path + " (" + fs.getSize(file) + " bytes)");
  ...
  private void hash(String path) throws IOException {
    try {
      ...
      // TODO RANG
      con.sendResponse(
          213, String.format("%s 0-%s %s %s", hash, fs.getSize(file), hex, fs.getName(file)));
    } catch (NoSuchAlgorithmException ex) {
      con.sendResponse(504, ex.getMessage());
    }
  }
