...
public class FTPServer implements Closeable {
  ...
  protected final List<FTPConnection> connections =
      Collections.synchronizedList(new ArrayList<FTPConnection>());
  protected final List<FTPListener> listeners =
      Collections.synchronizedList(new ArrayList<FTPListener>());

  protected UserAuthenticator auth = null;
   ...
   */
  public FTPServer(UserAuthenticator auth) {
   ...
   *
   * <p>Not only you can have your own user database, but you can also provide a different file
   * system depending on the user.
   *
   * @param auth The authenticator
   * @see NoOpAuthenticator
   */
  public void setAuthenticator(UserAuthenticator auth) {
   ...
   *
   * <p>This is required for supporting TLS/SLL.
   *
   ...
   *
   * <p>A server in explicit mode will support both secure and insecure connections. A server in
   * implicit mode will only support secure connections.
   *
   * <p>In order to support SSL, a context must be given with {@link #setSSLContext(SSLContext)}
   *
   ...
   *
   * <p>Connections that are idle (no commands or transfers) for the specified time will be
   * disconnected.
   *
   * <p>The default and recommended time is 5 minutes.
   *
   ...
   *
   * <p>The default value is 1024 bytes
   *
   ...
   *
   * <p>Connections to the server will still create new threads.
   *
   * @param port The server port
   * @throws IOException When an error occurs while starting the server
   */
   ...
   *
   * <p>Feel free to override this method with your own custom implementation
   *
   ...
   *
   * <p>For a complete cleanup, use {@link #close()} instead
   */
