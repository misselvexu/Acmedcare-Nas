 ...
 */
@SuppressWarnings({"deprecation", "serial", "ALL"})
public class ProxyServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
    ...
    String doLogStr = proxyConfig.getLog();
    if (doLogStr != null) {
  ...
  protected void initTarget() throws ServletException {
    ...
    targetUri = proxyConfig.getTargetUrl();
    if (targetUri == null) {
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
    ...
    try {
      ...
      if (statusCode == HttpServletResponse.SC_NOT_MODIFIED) {
        ...
        copyResponseEntity(
            proxyResponse, servletResponse, proxyRequest, servletRequest, requestUrl);
      }
  ...
  protected void copyResponseEntity(
    ...
    if (entity != null) {
      // TODO process response with wrapper
      // Content-Type application/json
      Header[] headers = proxyResponse.getHeaders("Content-Type");
      boolean wrapper = false;
      if (headers != null && headers.length > 0) {
        for (Header header : headers) {
          if (StringUtils.equalsAny(header.getValue(), "application/json")) {
            wrapper = true;
            break;
          }
        }
      ...
      if (wrapper) {
        byte[] content = ByteKits.input2byte(entity.getContent());
        ...
        try {
          JSONObject contentObject = JSON.parseObject(res);
          int code = 0;
          if (contentObject.containsKey("error")
              && StringUtils.isNoneBlank(contentObject.getString("error"))) {
            code = -1;
          }
          ...
          // build error
          BizResult errorResult =
              BizResult.builder()
                  .code(-1)
                  .exception(
                      ExceptionWrapper.builder()
                          .message("Acmedcare+ FS Upload File Response is Empty~")
                          .build())
                  .build();
          servletResponse.setHeader("Content-Length", String.valueOf(errorResult.bytes().length));
          servletOutputStream.write(errorResult.bytes());
        }
